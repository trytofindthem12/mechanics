...
public class CourseArray {
    ...
    public CourseArray(int numOfCourses, int numOfSlots) {
        ...
        slots = new int[numOfCourses]; // Initialize slots array
        Arrays.fill(slots, 0); // Set all slots initially to 0
    ...
    public void iterateExam(int examIndex, int shiftAmount) {
        if (examIndex >= 0 && examIndex < slots.length) {
            int currentSlot = slots[examIndex]; // Get the current slot for the exam
            int newSlot = currentSlot + shiftAmount; // Calculate the new slot

            // Ensure the new slot is within valid range (0 to numSlots - 1)
            if (newSlot >= 0 && newSlot < slots.length) {
                slots[examIndex] = newSlot; // Update the slot for the exam
            } else {
                System.out.println("Invalid new slot for exam " + examIndex);
            }
        } else {
            System.out.println("Invalid exam index.");
        }
    }
}
